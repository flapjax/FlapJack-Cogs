...
class Defcon:
	...
	async def defconplus(self, ctx):
		...
		await self.post_defcon(str(self.settings[server.id]["defcon"]), member.nick)
	...
	async def setdefcon(self, ctx, level):
		...
		if level in valid_defcons:
			...
			await self.bot.say("Not a valid DEFCON level. Haven't you seen War Games?")
	...
	async def post_defcon(self, level, nick):
		if level == '5':
			...
			subtitle = 'No known threats to your self esteem exist at this time.'
			instructions = ''.join(["- Liberal partipaction in online games is encouraged\n",
			...
			instructions = ''.join(["- Inhale deeply through your nose and count to 5\n",
			...
			instructions = ''.join(["- Use extreme caution when playing ranked games\n",
			...
			instructions = ''.join(["- Do not participate in any online games\n",
									"- Log off all social media immediately\n",
									"- Take shelter outdoors until the all-clear is given"])
